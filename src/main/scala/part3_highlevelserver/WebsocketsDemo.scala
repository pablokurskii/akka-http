...
object WebsocketsDemo extends App {
  ...
  def websocketFlow: Flow[Message, Message, Any] = Flow[Message].map {
    case tm: TextMessage =>
      /*here is just concatenation of messages*/
  ...
  /*when some get received on localhost:8080 it will open websocket from html*/
  ...
  /*this flow pushes messages from socialFeed back to client every 2 seconds*/
